...
import { toVisibilityLabel } from '@/utils/members/memberUtils';
...
const PrivacyPage = () => {
  ...
  const { treeVisibility, fetchTreeVisibility } = useMemberStore();

  useEffect(() => {
    fetchTreeVisibility();
  }, [fetchTreeVisibility]);

  const visibilityLabel = toVisibilityLabel(treeVisibility?.visibility);
      ...
      <div className="w-full flex flex-col gap-9 px-4">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <span className="typo-body_reg12 text-gray-300">
              차단된 사용자는 차단한 사용자의 컨텐츠를 볼 수 없고, 좋아요,
              댓글을 보낼 수 없습니다.
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-2.5">
            <button
              onClick={() =>
                openModal('treeVisibilityModal', { position: 'bottom' })
              }
              className="flex justify-between items-center cursor-pointer"
            >
              ...
              <div className="flex items-center gap-3">
                <span className="typo-body_bold14 text-gray-700">
                  {/* 일부 공개 */}
                  {visibilityLabel}
                </span>
                <MiniRightChevronIcon className="w-6 h-6 text-gray-900" />
