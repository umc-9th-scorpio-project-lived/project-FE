...
const SetRepeatModal = ({ initialValue, onApply }: Props) => {
  ...
  const dateItems = useMemo(() => Array.from({ length: 31 }, (_, i) => i + 1), []);
  ...
  const result = useMemo(() => {
    ...
    return formatRepeatResult({ tab: "WEEK", weekDays, isEveryday, everyWeeks });
        ...
        <div>
          <div className="mt-1.5 pt-4 px-5">
            ...
            <div className="flex items-center gap-1 pb-4 pt-1 border-b-[0.5px] border-gray-700">
              ...
              />
              <span className="typo-body_reg12 text-gray-600">매달 마지막 날</span>
            </div>
          </div>

          {/* 날짜 선택 chip */}
          <>
            <div className="grid grid-cols-7 gap-1 flex-wrap min-h-[84px] px-5 pt-3 pb-7">
              {selectedDates.map((d) => (
                <div
                  key={`date-chip-${d}`}
      ...
      {tab === "WEEK" && (
        <div className="flex flex-col gap-5 pt-8 pb-9">
          ...
          <div className="flex flex-col gap-4">
            ...
            <WeekWheel value={everyWeeks} onChange={setEveryWeeks} min={1} max={5} />
          </div>
        </div>
      )}
